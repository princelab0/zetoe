...
import { Logo } from "@/components/home/web-research/search-results";
import {
  Timeline,
  TimelineContent,
  TimelineDot,
...
const StepSection = ({
  ...
  const t = useTranslations("proRender");
  return (
    <div className='flex flex-col'>
      ...
      >
        <div className='text-muted-foreground font-medium'>{t("search")}</div>
      ...
      >
        <div className='text-muted-foreground font-medium'>{t("results")}</div>
        <div className='flex flex-wrap gap-1'>
...
export const ProSearchRender = ({
  ...
  streamingPromptMode = false,
}: {
  streamingProResponse: AgentSearchFullResponse | null;
  streamingPromptMode?: boolean;
}) => {
  ...
  if (
    !streamingProResponse?.steps_details &&
    !streamingProResponse?.steps?.length === undefined
  ) {
    return streamingPromptMode ? <ProSearchSkeleton /> : null;
  }
