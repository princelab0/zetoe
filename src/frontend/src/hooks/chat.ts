...
export const useChat = () => {
  ...
  const { model, promptMode } = useConfigStore();

  const [streamingMessage, setStreamingMessage] = useState<ChatMessage | null>(
    null
  );
  const [streamingPromptMode, setStreamingPromptMode] = useState({
    Web: false,
    Research: false,
    Writing: false,
    Grammar: false,
  ...
  let steps_details: AgentSearchStep[] = []; //reasoning before message
  ...
  const handleEvent = (eventItem: ChatResponseEvent, state: ChatMessage) => {
    switch (eventItem.event) {
      ...
      case StreamEvent.STREAM_END:
        ...
        setStreamingPromptMode({ ...streamingPromptMode, [promptMode]: true });
  ...
  const { mutateAsync: chat } = useMutation<void, Error, ChatRequest>({
    ...
    mutationFn: async (request) => {
      ...
      const req = {
        ...
        thread_id: threadId,
        model,
        pro_search: promptMode === "Research", //request Research
      };
      await streamChat({
        request: req,
        onMessage: (event) => {
