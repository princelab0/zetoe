...
class QueryPlanStep(BaseModel):
    ...
    dependencies: list[int] = Field(
        ..., description="List of step ids that this step depends on information from", default_factory=list
    )


class QueryPlan(BaseModel):
    steps: list[QueryPlanStep] = Field(..., description="The steps to complete the query", max_length=4)


class QueryStepExecution(BaseModel):
    search_queries: list[str] | None = Field(
        ..., description="The search queries to complete the step", min_length=1, max_length=3
    )
...
async def ranked_search_results_and_images_from_queries(
    ...
    search_responses: list[SearchResponse] = await asyncio.gather(*(perform_search(query) for query in queries))
...
async def stream_pro_search_objects(
    ...
    query_plan = llm.structured_complete(response_model=QueryPlan, prompt=query_plan_prompt)
    print(query_plan)
    ...
    for idx, step in enumerate(query_plan.steps):
        ...
        if not is_last_step:
            ...
            query_step_execution = llm.structured_complete(response_model=QueryStepExecution, prompt=search_prompt)
            ...
            yield ChatResponseEvent(
                ...
                data=AgentSearchQueriesStream(queries=search_queries, step_number=step_id),
            )

            search_results, image_results = await ranked_search_results_and_images_from_queries(search_queries)
            search_result_map[step_id] = search_results
            image_map[step_id] = image_results

            yield ChatResponseEvent(
                event=StreamEvent.AGENT_READ_RESULTS,
                data=AgentReadResultsStream(results=search_results, step_number=step_id),
            )
            ...
            relevant_result_map: dict[int, list[SearchResult]] = {id: search_result_map[id] for id in dependencies}
            DESIRED_RESULT_COUNT = 12
            total_results = sum(len(results) for results in relevant_result_map.values())
            ...
            search_results = [result for results in relevant_result_map.values() for result in results]

            search_results = list({result.url: result for result in search_results}.values())
            ...
            if not is_local_model(request.model):
                related_queries_task = asyncio.create_task(generate_related_queries(query, search_results, llm))
            ...
            related_queries = await (
                related_queries_task if related_queries_task else generate_related_queries(query, search_results, llm)
            )
